...
def get_args_parser():
    ...
    # * Dataset
    parser.add_argument('--dataset', default='pdl1/21448', type=str)
...
def test():
    ...
    model.load_state_dict(torch.load('./checkpoint/pdl1_20230604_1.pth')['model'])
    model.eval()
    data_loaders = construct_dataset()
    evaluator = Evaluator(data_loaders['test'])
    evaluator.visual_analysis(model, output_dir='./vis_results/8870')
...
class Evaluator:
    ...
    def predict(self, model, images, apply_deduplication: bool = False):
        ...
        #print('reserved_index', reserved_index)
    ...
    def visual_analysis(self, model, output_dir='./vis_results/8870'):
        ...
        for i, (images, points, labels, lengths) in enumerate(self.data_loader):
            ...
            pd_points, pd_classes = self.predict(model, images, apply_deduplication=True)
            #print('pd_points', pd_points.shape, pd_points)
            #print('pd_classes', pd_classes.shape, pd_classes)
